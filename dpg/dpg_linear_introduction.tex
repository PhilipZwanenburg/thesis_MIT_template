...
and $\fspace{V}(\fe{V})$ is the volume test space. Noting that the test graph space is a subset of $(L^2(\Omega))^n$, it
has been shown that the PG methodology of~\autoref{sec:PG_aspect_of_DPG} is, in fact, a subset of this practical DPG
methodology where $L^2$-conforming test spaces are used. \makered{Comment along the lines of: This comes at the cost of
introducing trace unknowns over the interior volume boundaries?} We now briefly reproduce the proof of Demkowicz et
