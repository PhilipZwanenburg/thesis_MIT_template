...
trace unknowns over the interior volume boundaries, in addition to trace unknowns defined on the domain boundary, both
of which are subsequently denoted by $\hat{u}$.

We separate the bilinear form into the following components
\begin{align}
b(v,u) \coloneqq b(v,(w,\hat{w})) \coloneqq \bar{b}(v,w) + \left<\left< v,\hat{w} \right>\right>
\end{align}

where $\bar{b}(v,w)$ includes all terms present in the PG methodology outlined in~\autoref{sec:PG_aspect_of_DPG} and
$\left<\left< v,\hat{w} \right>\right>$ accounts for newly introduced terms arising as a result of the usage of the
broken tests space. Above, $w$ includes the solution component in $L^2$, $u$, as well as the trace, $\hat{u}$, on the
domain boundary while $\hat{w}$ includes only the trace on the internal volume boundaries. Following the previous
notation, the discrete solution variables are represented by $w_h \in \afspace{W}_h$ and $\hat{w}_h \in \ftspace{W}_h$,
volume and trace approximate solution spaces. Defining the weakly conforming optimal test space as
\begin{align}
...
\begin{proposition}[PG Test Space as a Subset of DPG Test Space] \label{prop:pg_subset_of_dpg}
$\bar{V}_h \subset \afspace{V}_h.$
\end{proposition}

\begin{proof}
We briefly reproduce the proof of Demkowicz et al.~\cite[Section \makeblue{6}]{Demkowicz2014_overview}.
\end{proof}
